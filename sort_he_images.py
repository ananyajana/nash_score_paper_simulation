...
output_path ='/dresden/users/aj611/experiments/biomed/he_images_check/'
...
oversample_dict = {
        ...
        'nas_stea':{'fold_2':['0_2x'], 'fold_3':['0_2x']},
        'nas_lob':{'fold_2':['0_2x']},
        'nas_balloon':{'fold_1':['0_2x', '1_2x'], 'fold_2':['0_3x']}
        }
    ...
    for j in range(len(pat_idx)):
        ...
        for k in range(len(HE_files)):
            ...
            for exp in ['fib', 'nas_stea', 'nas_lob', 'nas_balloon']:
                ...
                full_dest_path = output_path + exp + '/' + fold_str + path_str + '/{0}/'.format(label) + img
                print(f'copying {full_src_path} to {full_dest_path}\n')
                message += f'\ncopying {full_src_path} to {full_dest_path}\n'
                shutil.copyfile(full_src_path, full_dest_path)
                if fold_str in oversample_dict[exp]:
                    # the format of this string is 0_2x where 0 is the class name which need to be repeated 2 times
                    oversample_list = oversample_dict[exp][fold_str]
                    for m in range(len(oversample_list)):
                        oversample_str = oversample_list[m]
                        oversample = oversample_str.split('_')
                        ovs_class = int(oversample[0])
                        ovs_num = int(oversample[1][:-1])
                        #print(f'exp :{exp} oversample string: {oversample_str}, ovs_class: {ovs_class}, ovs_num: {ovs_num}')
                        if fold_str in full_dest_path and (ovs_class == label):
                            for y in range(ovs_num - 1):
                                ovs_path = full_dest_path + '_{}'.format(y + 1)
                                shutil.copyfile(full_src_path, ovs_path)
